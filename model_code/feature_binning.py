...
class FeatureBinning(object):
    ...
    def calc_bin_gb(self, df, col_name, target_name, calc_badrate=False):
        ...
        '''
        # df_tmp = df.copy()
        df_grp = df.groupby([col_name])[target_name].agg(['count', 'sum'])
    ...
    def manual_bin(self, df, target='target', bins_dict=dict()):
        ...
        # 获取手动分箱前的var_bin_woe_dict
        self.get_var_bin_woe()

        var_summary = self.calc_var_summary(data_bin, target=target)
        manual_var_bin_woe_dict = var_summary[['var_name', 'range', 'woe']].groupby('var_name').apply(
            lambda df: df.set_index('range')['woe'].to_dict()).to_dict()
        print('手动分箱的结果：', manual_var_bin_woe_dict)
        print('原来的结果：', self.var_bin_woe_dict)

        # 更新手动分箱后的数据
        self.var_bin_woe_dict.update(manual_var_bin_woe_dict)
  ...
  Interval(4.5, inf, closed='left'): 1.7735952385975287}
    ...
    def transform(self, df, target='target', bins_dict={}):
        ...
        if target in df.columns:
            ...
            data_bin = pd.concat([i[0].astype(str) for i in data_with_bins], axis=1)
        b_dict = dict([(i[0].name, i[1].tolist()) for i in data_with_bins])
        if not operator.eq(bins_dict, b_dict):
            assert '传入的分箱和应用后的分箱不对等，请联系开发者'
    ...
    def bin_method_select(self, df, x_list=[], target='target', method='dt', **kwargs):
        ...
        if method == 'dt':
            return self.dt_bin(df, x_list=x_list, target=target, **kwargs)
        elif method == 'chi':
            return self.chi_bin(df, x_list=x_list, target=target, **kwargs)
        elif method == 'equal_freq':
            return self.equal_freq_bin(df, x_list=x_list, target=target, **kwargs)
        elif method == 'kmeans':
            return self.KMeans_bin(df, x_list=x_list, target=target, **kwargs)
